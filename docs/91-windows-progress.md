...
### 2026-07-23 更新之四十（**连接升级端到端跑通：RTT 293ms → 1.53ms；并纠正一个我下错的结论**）

#### ⚠️ 先纠正：上一轮我把 bug 归因成「锁屏」是错的
我说「升级后 recv=0 是因为机器锁屏采集不到画面，做过对照实验」。机器解锁后重测，**问题依旧**——所以那是真 bug，只是被锁屏这个同时存在的现象掩盖了。
对照实验本身没错（纯直连当时确实也 0 帧），但**我用它证明了「不是升级的锅」，这个推论过头了**：两个原因可以并存，排除其一不等于排除全部。**归因要等到真正修好、能复现「修改前坏 / 修改后好」为止。**

#### 逐层挖出的四个 bug（都是「看起来成功但没画面」）
1. **探测连接触发了真会话**：探测发的 HELLO 让 sender 起了完整会话，切换后我又发一次 HELLO，sender 再起一个 → 日志里**三个会话并存**，帧发到没人收的那条上。
   → 改为**回放探测期收到的帧**（HELLO_ACK 已经在里面），不再重新握手。这个模式我在 `connectAuto` 里用过，升级路径漏了。
2. **顶替时机太早**：`server.createServer` 里一有连接就 `active.stop()`，而探测连接只是试探——中转会话在对端切过来之前就被杀了。
   → 顶替移到**收到 HELLO 之后**（HELLO 才是「我要用这条」的意图声明）。
3. **`#hint` 元素被我删了但 renderer 还在用** → `startStreaming` 抛 `Cannot read properties of null`，`display` 虽然设上了但后续初始化中断。
   → 补回元素。**顺手写了个检查**：扫描 renderer 里所有 `$("id")` 与 HTML 实际 id 比对，这类「改 UI 删元素、JS 悄悄崩」的问题以后能一眼查出。
4. **虚拟网卡地址被当成可直连地址公布**：`28.0.0.1`（Mihomo TUN）被选中，流量绕回代理，**「升级到直连」后 RTT 仍是 293ms，等于白升级**。
   → `lanAddrs` 只公布**真实网卡**地址。虚拟网卡对端根本路由不到，公布它是错的。

#### 最终实测
```
[recv] 尝试直连升级 → 192.168.50.40:47800
[recv] ✅ 已升级到直连 192.168.50.40:47800（原中转连接关闭）
RECV_STATS {"codec":"hevc422","width":2560,"height":1600,"recv":669,"decoded":301,
            "errors":0,"keyframes":6,"avgRttMs":1.53}
```
**RTT 293ms → 1.53ms**（本机自连，跨机会是局域网真实值）。这正是新交互模型的意义：用户什么都没选，程序自己从中转升到了直连。

`dropped=349` 是背压丢帧——升级瞬间链路带宽突变（中转限速 → 局域网满速），编码器还按旧节奏发。稳态不涨，属预期；跨机验证时再看是否要调。
...
## 更新之四十一：按用户交付的设计重做界面（2026-07-23）

用户找设计师产出了统一界面设计（`docs/design/`），取代我和 Mac 各自吵过的两版草稿。已推 main（e1bb3f4 / 6ee7c70）。

### 架构：没有拆引擎，而是反转了归属

设计要求接收画面走独立窗口，但 socket、解码器、canvas 全在那个同时画着设置表单的窗口里。抽引擎风险太高（连接升级、背压补关键帧都是调很久才对的），所以反过来：**原来那个窗口保留全部引擎、自己变成接收窗口**，平时隐藏，对方投过来才显示。主面板和托盘是新窗口，只渲染状态发命令，不碰 socket。契约见 `windows/src/UI-CONTRACT.md`。

引擎原本直接从 DOM 读设置（`$("res").value`），表单没了这些读取就没了着落，改成 prefs 对象 + 持久化设备列表。

### 配对模型变了，且是硬性互通约定

设计要求「两端输入同一个 6 位码」，房间号不再由 relay 分配而是两端各自算。推导写进了协议 §3.7 并给了自检向量——差一个字节就会各自进不同房间，而且**两边日志都正常**，极难查。这是本次唯一需要 Mac 逐字节对齐的东西。

### 删掉了「等待对方连接我 / 我去连接对方」

用户这周实测就栽在这儿：他被问的是「谁拨谁」，脑子里想的是「谁投给谁」，于是把 Windows 也摆成投射方，两边都在等、没人 join。方向现在由 role/recvSvc 决定，谁 listen 谁 dial 内部按 deviceId 字典序定，用户看不到。

### 三个坑

**给界面的初始状态快照不能是 null。** panel/tray 一起来就 `nd-state-get`，那时引擎还没推过状态。给 null 它们在 `state.role` 上抛异常，而异常在**首次绘制之前** → `ready-to-show` 永不触发 → **窗口根本不出现**。症状：进程活着、三个 renderer 都加载了、日志一行错都没有、桌面上什么都没有。现在主进程发结构完整的空状态，且 3s 后无论如何显示面板——宁可显示画坏的窗口，也不能让程序看起来根本没启动。

**两个写界面的 agent 独立犯了同一个错**：都把画质值发成中文标签。全角 `×` 被 `split("x")` 拆成 NaN，一路走到编码器才炸。两边同时犯说明不是他们的问题，是契约没写死——已补取值表。

**我自己踩了一次编码坑**：用 `Get-Content -Raw | Set-Content` 对 renderer.js 做全文替换，PS 5.1 按 ANSI 读 UTF-8 文件，中文全成乱码且**有字节丢失**（连反引号都被吃掉），不可逆。已从 git 恢复重做。此前记的规则是「.ps1 要 BOM、.json 不要 BOM」，这次是同一个陷阱的另一个方向：**别用 PowerShell 对含中文的源文件做全文读写**，用 Edit 工具或 node。同理，别在 bash 的 `node -e` 里写含反引号的模板字符串——反引号会被 shell 抢先解释，代码块会静默变成空白（这一条就是写这段时踩的）。

### 验证

同码撮合 → 握手 → 364 帧 0 错，且自动升级直连（RTT 1.55ms）；CLI 联调参数全保留（`--send-relay` / `--recv-relay` / `--secret` / `--relay <码>`）；role 编排测试 20/20；主面板实跑截图核对设计稿。未做跨机真机验证。
