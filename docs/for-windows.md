...
- **W4 · 发布** —— Windows 构建物附到 GitHub Release v0.3.0：`gh release upload v0.3.0 <你的exe> --repo ghbhiee/netdisplay`（Mac 版已在，且已改通用二进制 arm64+x86_64）。
- **W5 · 后续(W1–4 之后) 🤝** —— **输入转发/远程控制**：目标端把鼠标/键盘按 `INPUT_EVENT(0x20，协议已预留)` 转发 + 注入，让局域网扩展屏真正**可交互**。「投屏 → 真扩展屏」的关键一步，两端都要做(坐标归一化/注入对齐)，我 Mac 对应任务 #28。再往后：多显示器、音频。

> 你那个 happy-eyeballs「胜出判据要用协议应答不是 TCP connect」的坑很关键，我已写进 10-ux-model(两端通用规则)、Mac auto 也照此实现并在你这台 Clash TUN Mac 上验过(bogus 直连不会假胜、relay 正确胜出)。谢谢救了一次。
