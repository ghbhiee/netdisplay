...
>
> Changelog:
> - 2026-07-25 v1.16 **画质只归投射方 + 分辨率规则 + 呈现模式 `mode`（§3.10）**：画质设置**只保留在投射方**（接收端不再有，接收端改也没用——它的分辨率请求本来就被发送方覆盖）。规则：**跟随对方屏幕**=用对方 HELLO 报的屏幕分辨率、`mode="extend"`（对方按第二显示器/全屏呈现）；**用户指定分辨率**=按用户的来但**逐轴 clamp、绝不超过对方屏幕**（超了自动修正），`mode="window"`（对方用窗口呈现）；**单窗口投射永远 `mode="window"`**。`mode` 加在 `HELLO_ACK.display.mode` 与 `VIDEO_CONFIG.mode`，缺省=`extend`（老端兼容）。**同时移除接收端把 `HELLO_ACK.pairSecret` 写回设备 secret 的老逻辑**——新模型里 secret=配对码推出的房间钥匙，覆盖它会把设备悄悄搬到别的房间，导致「第一次投射成功、之后永远配不上」（Windows 实测踩中，Mac 同样路径一并删除）。（两端 Claude）
...
### 3.10 画质与分辨率规则（v1.16，**两端必须一致**）

**画质设置只属于投射方**（发送端拥有虚拟屏 + 编码器）。接收端**不要**提供画质设置——
它请求的分辨率本来就会被发送方覆盖，摆在那里只会互相冲突、让用户困惑。

发送方按下面这一条规则算出串流分辨率与**呈现模式** `mode`：

| 用户在投射方选的 | 串流分辨率 | `mode` | 接收方怎么显示 |
|---|---|---|---|
| **跟随对方屏幕**（默认） | 对方 `HELLO.screen.width/height` | `extend` | 当作第二显示器/全屏 |
| **指定分辨率** | 用户值，**逐轴 clamp 到不超过对方屏幕**（超出自动修正） | `window` | 普通窗口 |
| **单窗口投射** | 该窗口自身像素尺寸（同样不超过对方屏幕） | `window` | 普通窗口 |

理由：比对方面板还小的「显示器」只有窗口化才讲得通；而只有「跟随对方屏幕」才配得上
真正的扩展屏。分辨率必须偶数（编码器要求），奇数向下取偶。

**`extend` 的串流分辨率取决于发送端能不能自己定画面尺寸**（v1.16 补充，两端已对齐）：

- **能创建虚拟显示器的发送端**（Mac 的 `CGVirtualDisplay`）：**把虚拟屏建成对方屏幕的尺寸**，
  于是原生就在该分辨率渲染+编码，**全程不缩放**，这是真正的扩展屏。
- **只能抓现有物理屏的发送端**（Windows 尚未做虚拟显示器前）：**按原生分辨率发，不要为了
  凑对方分辨率去缩放**——尤其**绝不允许放大**（对方屏比自己大时放大只是浪费带宽、不增细节）。
  仍然标 `mode="extend"`，由接收端全屏呈现。

**接收端职责**：把收到的画面**等比适配**到呈现区域（`extend`=全屏/整块扩展屏，`window`=窗口），
**保持宽高比、必要时加黑边，绝不拉伸变形**。因为两端屏幕比例本来就可能不同，这个适配步骤
在任何一种模式下都必须有。

也就是说：`mode` 约定的是**接收端怎么呈现**，分辨率只是**流的实际像素尺寸**；发送端「跟随
对方屏幕」是一条**在能做到时才照做**的尺寸目标，做不到就发原生，语义不变。

`mode` 放在 `HELLO_ACK.display.mode`，中途改画质时放在 `VIDEO_CONFIG.mode`；
**字段缺省 = `extend`**（老发送端兼容）。接收端必须容忍缺省与未知值（未知按 `extend`）。

Mac 端自检：`netdisplay-sender resolution-selftest`（跟随/不裁剪/超限修正/偶数化四条）。
